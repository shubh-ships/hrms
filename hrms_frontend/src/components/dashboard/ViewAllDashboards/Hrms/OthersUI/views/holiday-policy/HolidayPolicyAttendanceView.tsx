...
const attendanceTypeOptions = [
  "Assign Overtime",
  "Assign Regular Payable Day",
  "Assign Comp-Off",
];
const rateTypeOptions = ["Salary Multiplier", "Fixed Rate"];

// --- Mapping helpers ---
const typeToApi = (t: string): string => {
  if (t === "Assign Overtime") return "OVERTIME";
  if (t === "Assign Comp-Off") return "COMP_OFF";
  return "REGULAR_PAYABLE_DAY";
};
const typeFromApi = (t: string): string => {
  if (t === "OVERTIME") return "Assign Overtime";
  if (t === "COMP_OFF") return "Assign Comp-Off";
  return "Assign Regular Payable Day";
};
const rateToApi = (r: string): string => (r === "Fixed Rate" ? "FIXED_AMOUNT" : "SALARY_MULTIPLIER");
const rateFromApi = (r: string): string => (r === "FIXED_AMOUNT" ? "Fixed Rate" : "Salary Multiplier");
...
// --- Main Component ---
const HolidayPolicyAttendanceView = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await axiosClient.get(`/attendanceonholiday-templates/${id}`);
        const t = res.data?.data;
        if (!t) return;
        setTemplateName(t.name || "");
        setAttendanceType(typeFromApi(t.attendanceOnHolidayType || "REGULAR_PAYABLE_DAY"));
  ...
  const handleSave = async () => {
    ...
    const payload: Record<string, any> = {
      ...
      attendanceOnHolidayType: apiType,
      disableAutomationRules: ignoreAutomationRules,
    };

    // Only send compensation fields for OVERTIME — backend auto-sets for others
    if (apiType === "OVERTIME") {
    ...
    try {
      if (editingId) {
        await axiosClient.put(`/attendanceonholiday-templates/${editingId}`, payload);
        sessionStorage.removeItem("editHolidayPolicyId");
      } else {
        await axiosClient.post("/attendanceonholiday-templates", payload);
      }
      ...
      <div className="p-4 md:p-6 max-w-[1400px] w-full mx-auto">
        <div className="bg-white rounded-xl border border-gray-200 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)] p-6">
          <h2 className="text-[17px] font-bold text-slate-800 mb-6">
            Attendance on holiday configuration
          </h2>
          ...
          <div className="max-w-[660px] bg-[#f8f9fa] rounded-xl border border-gray-100 p-5 mt-6">
            ...
            <p className="text-[11px] text-gray-500 font-medium mb-5">Choose the attendance type to assign for working on a holiday</p>
            ...
            <>
              <div className="max-w-[660px] bg-white rounded-xl border border-gray-100 mt-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.02)]">
                <div className="p-5 border-b border-gray-50">
                  <p className="text-[14px] font-bold text-slate-800 mb-1">Holiday Compensation</p>
                  <p className="text-[11px] text-gray-500 font-medium">Configure compensation structure for working on holiday dates</p>
                </div>
              ...
              {/* Ignore automation rules toggle */}
              <div className="mt-8 flex items-center gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex items-center gap-2 relative">
                  <span className="text-[14px] text-slate-800 font-medium tracking-tight">Ignore automation rules on holidays</span>
                  ...
                  {showTooltip && (
                    <div className="absolute bottom-full mb-2 left-[120px] -translate-x-1/2 w-[240px] bg-[#2d3748] text-white text-[11px] p-3 rounded-lg shadow-xl animate-in fade-in zoom-in-95 duration-150 z-50 leading-relaxed">
                      Automation rules will not apply on holidays; working hours will be entirely calculated as overtime
...
export default HolidayPolicyAttendanceView;
