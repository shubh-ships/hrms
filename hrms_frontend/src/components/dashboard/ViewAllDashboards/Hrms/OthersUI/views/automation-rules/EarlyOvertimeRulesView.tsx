...
import { fromApiRulesOvertime, toApiRulesOvertime, defaultOccConfig, OccurrencesConfig } from "./automationApi";
import AutomationStaffModal from "./AutomationStaffModal";

const API_TYPE = "early_overtime";
const INCLUDE_CALC_TYPE = false;
const RULE_TITLE = "Early Overtime Rules";
const RULE_DESC = "Automate overtime for employees working extra hours before their shift start time";
...
const getMinutesText = (h: string, m: string) => {
  ...
  return t === 0 ? "No overtime for 0 mins" : `Not applicable upto ${t} mins`;
};
...
const CustomCalculationTypeSelect = ({ value, onChange, width = "280px" }: any) => {
  const [isOpen, setIsOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  useClickOutside(ref, () => setIsOpen(false));
  const options = ["Post Payable Hours", "Post Payable Hours and Shift End", "Post Payable Hours or Shift End", "Shift End"];
  return (
    <div className="relative" ref={ref} style={{ width }}>
      <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between bg-white border border-gray-200 rounded-lg py-2 px-4 focus:outline-none focus:border-[#3f5a54] h-[42px]">
        <span className="text-[14px] text-slate-800">{value}</span>
        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      {isOpen && (
        <div className="absolute z-50 mt-1 bg-white border border-gray-100 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.1)] overflow-hidden py-2 w-full">
          {options.map((opt) => (
            <button key={opt} type="button" onClick={() => { onChange(opt); setIsOpen(false); }} className="w-full flex items-center justify-start py-2.5 px-4 hover:bg-gray-50 relative text-left">
              <div className="flex items-center gap-3">
                {value === opt ? <div className="w-2 h-2 bg-[#3f5a54] rounded-full shrink-0" /> : <div className="w-2 h-2 shrink-0" />}
                <span className={`text-[14px] ${value === opt ? 'text-slate-800 font-medium' : 'text-slate-600'}`}>{opt}</span>
              </div>
            </button>
          ))}
        </div>
...
export default function EarlyOvertimeRulesView() {
  const [ruleName, setRuleName] = useState("");
  const [calculationType, setCalculationType] = useState("Shift End");
  ...
  const handleEdit = (rule: any) => {
    const s = fromApiRulesOvertime(rule.rules);
    setRuleName(rule.name || "");
    setDeductSalary(s.deductSalary); setSalaryRanges(s.salaryRanges);
    setDeductHalfDay(s.deductHalfDay); setHalfDayTime(s.halfDayTime);
    setDeductFullDay(s.deductFullDay); setFullDayTime(s.fullDayTime);
    setIncludeLateFine(s.includeLateFine); setCalculationType(s.calculationType);
    setEditingRuleId(rule._id); setShowForm(true);
  };

  const handleSave = async () => {
    if (!ruleName.trim()) return;
    const state = { deductSalary, salaryRanges, deductHalfDay, halfDayTime, deductFullDay, fullDayTime, includeLateFine, calculationType };
    const payload = { type: API_TYPE, name: ruleName.trim(), rules: toApiRulesOvertime(state, INCLUDE_CALC_TYPE) };
    setIsSaving(true);
    try {
  ...
  const resetForm = () => {
    setRuleName(""); setCalculationType("Shift End");
    setDeductSalary(false); setSalaryRanges([{ id: "1", hours: "00", minutes: "00", deductionType: "Fixed Amount", fixedAmount: "0" }]);
    setDeductHalfDay(false); setHalfDayTime({ hours: "00", minutes: "00" });
    setDeductFullDay(false); setFullDayTime({ hours: "00", minutes: "00" });
    setIncludeLateFine(false); setEditingRuleId(null);
  };
    ...
    <div className="flex flex-col mt-4">
      ...
      <div className="flex items-start gap-6 mb-8">
        <div className="flex-1 max-w-[300px]">
          ...
          <input type="text" placeholder="Enter Name" value={ruleName} onChange={e => setRuleName(e.target.value)} className="w-full border border-gray-200 rounded-md py-2.5 px-3 text-[13px] text-slate-800 focus:outline-none focus:border-[#3f5a54] placeholder-gray-400 h-[42px]" />
        </div>
        {INCLUDE_CALC_TYPE && (
          <div>
            <label className="block text-[11px] font-medium text-gray-700 mb-1.5">Calculation Type <span className="text-red-500">*</span></label>
            <CustomCalculationTypeSelect value={calculationType} onChange={setCalculationType} />
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="space-y-3 pb-4">
          <label className="flex items-center gap-2 cursor-pointer w-fit">
            ...
            <span className="text-[13px] font-medium text-slate-700">Add salary based on overtime slabs</span>
          </label>
          {deductSalary && (
            <div className="bg-[#F8F9FA] p-5 rounded-xl border border-gray-100 flex flex-col gap-5">
              {salaryRanges.map((range) => (
                <div key={range.id} className="flex flex-wrap items-start gap-[18px]">
                  <div>
                    <div className="text-[11px] text-gray-600 mb-[7px]">If Overtime exceeds</div>
                  ...
                  <div>
                    <div className="text-[11px] text-gray-600 mb-[7px]">Rate Type</div>
        ...
        <div className="space-y-3 pb-4">
          <label className="flex items-center gap-2 cursor-pointer w-fit">
            ...
            <span className="text-[13px] font-medium text-slate-700">Add half day salary if overtime exceeds</span>
          </label>
          {deductHalfDay && (
        ...
        <div className="space-y-3 border-b border-gray-100 pb-8">
          <label className="flex items-center gap-2 cursor-pointer w-fit">
            ...
            <span className="text-[13px] font-medium text-slate-700">Add full day salary if overtime exceeds</span>
          </label>
          {deductFullDay && (
            <div className="bg-[#F8F9FA] p-5 rounded-xl border border-gray-100 flex flex-col gap-4">
              <div>
        ...
        <div className="pt-2 flex items-center gap-3">
          ...
          <span className="text-[13px] font-medium text-slate-600">Include overtime from start time of shift</span>
        </div>
      </div>
      <div className="h-24" />
