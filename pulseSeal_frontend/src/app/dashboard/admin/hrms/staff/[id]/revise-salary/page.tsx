...
export default function ReviseSalaryPage() {
    ...
    React.useEffect(() => {
        ...
        const fetchCurrent = async () => {
            try {
                ...
                const res = await dispatch(fetchPayrollOverview({ employeeId: id, month: today.getMonth() + 1, year: today.getFullYear() })).unwrap();
                if (res && res.payroll) {
                    const ctc = res.payroll.monthlyCTC || 0;
                    const b = res.payroll.basic || 0;
                    setFormState(prev => ({
                        ...prev,
                        currentCTC: ctc.toString(),
                        monthlyAmount: ctc.toString(),
                        basicSalary: b.toString(),
                        template: res.payroll.templateName || "Select Template"
                    }));
                }
            } catch(e) {
                console.error("Failed to fetch active structure", e);
            }
        };
        fetchCurrent();
    }, [id, dispatch]);
    ...
    const handleTemplateSelect = async (t: any) => {
        ...
        try {
            ...
            const ctc = parseFloat(formState.monthlyAmount) || parseFloat(formState.currentCTC) || 0;
            const basic = ctc * 0.5; // Default assumption
            setFormState(prev => ({
                ...prev,
    ...
    const handleCTCChange = (ctc: string) => {
        ...
        const p = parseFloat(formState.reviseByPercent) || 0;
        const newAmount = c + (c * p / 100);
        setFormState({
            ...formState,
            currentCTC: ctc,
            monthlyAmount: newAmount.toFixed(2),
            basicSalary: newAmount.toFixed(2)
        });
    };

    const handlePercentChange = (percent: string) => {
        const p = parseFloat(percent) || 0;
        const current = parseFloat(formState.currentCTC) || 0;
        const newAmount = current + (current * p / 100);
        setFormState({
            ...formState,
            reviseByPercent: percent,
            monthlyAmount: newAmount.toFixed(2),
            basicSalary: newAmount.toFixed(2)
        });
    };
    ...
    const handleAmountChange = (amount: string) => {
        ...
        const current = parseFloat(formState.currentCTC) || 0;
        const newPercent = current > 0 ? ((a / current) - 1) * 100 : 0;
        setFormState({
            ...formState,
            monthlyAmount: amount,
            reviseByPercent: newPercent.toFixed(2),
            basicSalary: amount
        });
    };
    ...
    const handleFinalSubmit = async () => {
        try {
            const currentSalary = parseFloat(formState.monthlyAmount) || 0;
            const basic = parseFloat(formState.basicSalary) || 0;
            const percentChange = parseFloat(formState.reviseByPercent) || 0;

            await dispatch(reviseSalary({
                employeeId: id,
                percent: percentChange,
                newCTC: currentSalary,
                newBasic: basic
            })).unwrap();

            router.push(`/dashboard/admin/hrms/staff/${id}?tab=Salary Structure`);
        } catch(e) {
            console.error("Failed to revise salary", e);
        }
    };
        ...
        <div className="flex flex-col min-h-screen bg-[#F9FAFB] font-sans">
            <div className="p-[40px] flex-1 flex flex-col">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-8 mb-6 shadow-sm max-w-[1128px]">
                    <h1 className="text-[20px] font-semibold text-[#1F2937] mb-8">Revise Salary Structure</h1>
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm max-w-[1128px]">
                    {/* Top Section with specific inputs */}
                    <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="flex flex-col gap-1.5 w-[256px]">
                            <label className="text-[7px] font-regular text-[#4B5563] uppercase tracking-tight">Current CTC</label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full h-[36px] bg-white border border-gray-200 rounded-lg pl-6 pr-4 text-[10px] text-[#1F2937] outline-none"
                                />
                            </div>
                        </div>

                        <div className="flex flex-col gap-1.5 w-[256px]">
                            <label className="text-[7px] font-regular text-[#4B5563] uppercase tracking-tight">Revise by Percent</label>
                            <div className="relative">
                                <input
                                    type="text"
                                    value={formState.reviseByPercent}
                                    onChange={(e) => handlePercentChange(e.target.value)}
                                    placeholder="0"
                                    className="w-full h-[36px] bg-white border border-gray-200 rounded-lg px-4 text-[10px] text-[#1F2937] outline-none focus:border-[#3B82F6]"
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[#9CA3AF]">%</span>
                            </div>
                        </div>
                                ...
                                <div className="w-[20%] px-4">
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            readOnly
                                            value={(parseFloat(formState.basicSalary) * 12).toFixed(2)}
                                ...
                                <div className="w-[20%] px-4">
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            readOnly
                                            value={Math.max(0, (parseFloat(formState.monthlyAmount) || 0) - (parseFloat(formState.basicSalary) || 0)).toFixed(2)}
                                ...
                                <div className="w-[20%] px-4">
                                    <div className="relative">
                                        ...
                                        <input
                                            type="text"
                                            readOnly
                                            value={(Math.max(0, (parseFloat(formState.monthlyAmount) || 0) - (parseFloat(formState.basicSalary) || 0)) * 12).toFixed(2)}
                                            className="w-full h-[36px] bg-gray-50 border border-gray-200 rounded-lg pl-6 pr-4 text-[10px] text-[#1F2937] outline-none cursor-not-allowed"
                                        />
                                    </div>
                                </div>
