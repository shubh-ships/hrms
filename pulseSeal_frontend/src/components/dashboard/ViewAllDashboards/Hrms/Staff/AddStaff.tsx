...
import ReplaceWithNewUserModal from "./ReplaceWithNewUserModal";
import ReplaceWithExistingUserModal from "./ReplaceWithExistingUserModal";
import CreateAdminModal from "./CreateAdminModal";

import { useAppDispatch, useAppSelector } from "@/store/hooks";
import { createAdmin, getAdminUsers, selectAdmins, selectAdminLoading, replaceUserRole, updateAdmin, deleteAdmin, AdminUser } from "@/features/newUser/newUserSlice";
...
export default function AddStaff() {
    ...
    const resolveReportsTo = (pObj: any) => {
        ...
        const empMatch = (employees || []).find(e => {
            const userId = typeof (e as any).userId === 'object' ? ((e as any).userId as any)?._id : (e as any).userId;
            return String(e._id) === pId || String(userId) === pId;
        });
    ...
    const handleEditSave = async (updatedUser: any) => {
        try {
            if (updatedUser.userType === "Admins") {
                const adminPayload = {
                    name: updatedUser.name,
                    email: updatedUser.email,
                    phoneNumber: updatedUser.phone
                };
                const resultAction = await dispatch(updateAdmin({ id: updatedUser.userId, adminData: adminPayload }));
                if (resultAction.meta.requestStatus === "fulfilled") {
                    toast.success("Admin updated successfully");
                    dispatch(getAdminUsers());
                    setIsEditModalOpen(false);
                } else {
                    toast.error(typeof resultAction.payload === 'string' ? resultAction.payload : "Failed to update admin");
                }
                return;
            }
    ...
    const handleRemoveUser = async (listId: string, coreUserId: string, userType?: string) => {
        try {
            if (!coreUserId) {
            ...
            if (userType === "Admins") {
                await dispatch(deleteAdmin(coreUserId)).unwrap();
    ...
    const handleDeactivateUser = async (userId: string, userType?: string) => {
        try {
            ...
            if (resultAction.meta.requestStatus === "fulfilled") {
                toast.warning(`${userType === "Admins" ? "Admin" : "User"} deactivated successfully`);
            } else {
                toast.error(typeof resultAction.payload === 'string' ? resultAction.payload : "Deactivation failed");
            }
            // Refresh lists for full synchronization
            dispatch(fetchUsers(undefined as any));
            dispatch(getAdminUsers());
            dispatch(fetchEmployees({}));
        } catch (error) {
    ...
    const handleActivateUser = async (userId: string, userType?: string) => {
        try {
            const resultAction = await dispatch(updateUserStatus({ id: userId, isActive: true }));
            if (resultAction.meta.requestStatus === "fulfilled") {
                toast.success(`${userType === "Admins" ? "Admin" : "User"} activated successfully`);
            } else {
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-[24px] w-full mt-[24px]">
                    ...
                    <div className="bg-white border border-[#E5E7EB] rounded-xl p-5 flex flex-col shadow-sm h-[130px]">
                        ...
                        <span className="text-[14px] font-medium text-[#0F172A]">{totalUsersCount}</span>
                    </div>
                    ...
                    <div className="bg-white border border-[#E5E7EB] rounded-xl p-5 flex flex-col shadow-sm h-[130px]">
                        ...
                        <span className="text-[14px] font-medium text-[#0F172A]">{activeUsersCount}</span>
                    </div>
                                                            ...
                                                            <div className={cn(
                                                                ...
                                                                {user.status === "Active" ? (
                                                                    <>
                                                                        ...
                                                                        >
                                                                            Edit {user.userType === "Admins" ? "Admin" : "User"}
                                                                        </button>
                                                                        ...
                                                                        <button
                                                                            onClick={() => (handleRemoveUser as any)(user.id, user.userId, user.userType)}
                                                                            className="w-full text-left px-5 py-2.5 text-[11px] font-medium text-[#EF4444] hover:bg-[#F9FAFB] transition-colors duration-200"
                                                                        >
                                                                            {user.userType === "Admins" ? "Delete Admin" : "Remove from Role"}
                                                                        </button>

                                                                        <button
                                                                            onClick={() => handleDeactivateUser(user.userId, user.userType)}
                                                                            className="w-full text-left px-5 py-2.5 text-[11px] font-medium text-[#4B5563] hover:bg-[#F9FAFB] transition-colors duration-200"
                                                                        >
                                                                            Deactivate
                                                                        </button>
                                                                    ...
                                                                    <button
                                                                        onClick={() => handleActivateUser(user.userId, user.userType)}
                                                                        className="w-full text-left px-5 py-2.5 text-[11px] font-medium text-[#10B981] hover:bg-[#F9FAFB] transition-colors duration-200"
                                                                    >
                                                                        Activate {user.userType === "Admins" ? "Admin" : "User"}
                                                                    </button>
                                                                )}
                                                            </div>
                                                        </>
