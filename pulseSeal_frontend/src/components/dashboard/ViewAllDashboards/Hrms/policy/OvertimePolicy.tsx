...
import { Plus, Trash2, Save, Power, Timer, DollarSign } from "lucide-react";
import { toast } from "sonner";
import type { OvertimeRule, OvertimePayRule, CreatePolicyData } from "@/features/policySlice/policeSlice";

const OvertimePolicyComponent = () => {
  const dispatch = useAppDispatch();
  const policy = useAppSelector(selectPolicyByName("overtime"));
  ...
  const [formData, setFormData] = useState<{
    ...
    description: "",
    overtimeRules: [],
    calculationType: "post_payable_hours",
    addGraceMinutesToOvertime: false
  });
  ...
  useEffect(() => {
    if (policy) {
      setFormData({
        ...
        calculationType: policy.calculationType || "post_payable_hours",
        addGraceMinutesToOvertime: policy.addGraceMinutesToOvertime || false
      });
    }
  }, [policy]);
...
const handleSave = async () => {
  try {
    const policyData: CreatePolicyData = {
      ...
      calculationType: formData.calculationType as any,
      addGraceMinutesToOvertime: formData.addGraceMinutesToOvertime,
      overtimeRules: formData.overtimeRules.map(rule => {
        ...
        // Only include threshold fields for half_day_pay and full_day_pay
        if (rule.ruleType === "half_day_pay" || rule.ruleType === "full_day_pay") {
          return {
            ...baseRule,
        ...
        // For salary_pay, include overtimePay WITHOUT _id
        return {
          ...baseRule,
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Configure payment rules for overtime work
              </CardDescription>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData(prev => ({ ...prev, calculationType: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="post_payable_hours">Post Payable Hours</SelectItem>
                    <SelectItem value="post_payable_hours_and_shift_end">Post Payable Hours & Shift End</SelectItem>
                    <SelectItem value="post_payable_hours_or_shift_end">Post Payable Hours OR Shift End</SelectItem>
              ...
              <div className="flex items-center space-x-2 pt-6">
                <Switch
                  checked={formData.addGraceMinutesToOvertime}
                  onCheckedChange={(checked) => setFormData(prev => ({ ...prev, addGraceMinutesToOvertime: checked }))}
                />
                <Label>Add grace minutes</Label>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Overtime Payment Rules</CardTitle>
            ...
            {formData.overtimeRules.map((rule, ruleIndex) => (
              <Card key={rule._id} className="border-l-4 border-l-green-500">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <Input
                        placeholder="Rule name (e.g., Regular Overtime Pay)"
                        value={rule.ruleName}
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* Threshold fields only for half_day_pay and full_day_pay */}
                  {(rule.ruleType === "half_day_pay" || rule.ruleType === "full_day_pay") && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                  ...
                  {rule.ruleType === "salary_pay" && (
                    <div className="space-y-4">
                      <Separator />
                      <div className="flex items-center justify-between">
                        <Label className="text-base font-medium">Overtime Pay Rules</Label>
                        <Button
                          variant="outline"
                          size="sm"
...
export default OvertimePolicyComponent;
