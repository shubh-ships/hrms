...
// ---- Staff List Modal (API-connected for both template types) ----
const StaffListModal = ({
  isOpen, onClose, templateId, templateType, onDone,
}: {
  isOpen: boolean; onClose: () => void; templateId: string | null; templateType: "Holiday" | "Attendance On Holiday"; onDone: () => void;
}) => {
  ...
  const loadStaff = useCallback(async (tid: string) => {
    setLoading(true);
    try {
      const isAttendance = templateType === "Attendance On Holiday";

      let aRes, uRes;
      if (isAttendance) {
        // Uses GET /:templateId/staff?unassignedStaff=false|true
        [aRes, uRes] = await Promise.all([
          axiosClient.get(`/attendanceonholiday-templates/${tid}/staff`, { params: { unassignedStaff: false } }),
          axiosClient.get(`/attendanceonholiday-templates/${tid}/staff`, { params: { unassignedStaff: true } }),
        ]);
      } else {
        [aRes, uRes] = await Promise.all([
          axiosClient.get(`/holiday/${tid}/assigned`),
          axiosClient.get(`/holiday/${tid}/unassigned`),
        ]);
      }
      ...
      // Safely extract the data array since the backend returns { data: { data: [...] } }
      const assignedList = Array.isArray(aRes.data?.data?.data) ? aRes.data.data.data : (Array.isArray(aRes.data?.data) ? aRes.data.data : []);
      const unassignedList = Array.isArray(uRes.data?.data?.data) ? uRes.data.data.data : (Array.isArray(uRes.data?.data) ? uRes.data.data : []);

      setAssignedStaff(assignedList.map(mapEmp));
      setUnassignedStaff(unassignedList.map(mapEmp));
    } catch (err) {
  ...
  const handleMoveToSelected = async () => {
    ...
    setOperating(true);
    try {
      if (templateType === "Attendance On Holiday") {
        await axiosClient.put("/attendanceonholiday-templates/assign", { templateId, employeeIds: Array.from(checkedIds) });
      } else {
        await axiosClient.post("/holiday/assign", { employeeIds: Array.from(checkedIds), templateId });
      }
      setCheckedIds(new Set());
  ...
  const handleMoveToUnselected = async () => {
    ...
    try {
      if (templateType === "Attendance On Holiday") {
        await axiosClient.put("/attendanceonholiday-templates/remove", { employeeIds: Array.from(checkedIds) });
      } else {
        await axiosClient.post(`/holiday/${templateId}/remove`, { staffIds: Array.from(checkedIds) });
      }
      setCheckedIds(new Set());
      await loadStaff(templateId);
      onDone();
...
// ---- Main View ----
const HolidayPolicyView = () => {
  const [activeTab, setActiveTab] = useState<"Holiday" | "Attendance On Holiday">("Holiday");
  ...
  const [deleteType, setDeleteType] = useState<"Holiday" | "Attendance On Holiday">("Holiday");
  ...
  const fetchAll = useCallback(async () => {
    try {
      const [hRes, aRes] = await Promise.all([
        axiosClient.get("/holiday"),
        axiosClient.get("/attendanceonholiday-templates"),
      ]);
      const rawH = hRes.data?.data || [];
      setHolidayTemplates(rawH.map((t: any) => ({
        id: String(t._id),
        templateName: t.name,
        status: t.status || "ACTIVE",
        holidayCount: (t.holidays || []).length,
        assignedStaff: 0, // loaded lazily below
      })));
      const rawA = aRes.data?.data || [];
      setAttendanceTemplates(rawA.map((t: any) => ({
        id: String(t._id),
        templateName: t.name,
        attendanceType: attendanceTypeLabel(t.attendanceOnHolidayType),
        compensationType: t.compensationType,
        compensationValue: t.compensationValue,
      })));
    } catch (err: any) {
  ...
  // Fetch assigned staff counts for both types of templates
  useEffect(() => {
    if (holidayTemplates.length === 0 && attendanceTemplates.length === 0) return;

    const loadCounts = async () => {
      if (holidayTemplates.length > 0) {
        const updated = await Promise.all(
          holidayTemplates.map(async (t) => {
            try {
              const res = await axiosClient.get(`/holiday/${t.id}/assigned`);
              const count = Array.isArray(res.data?.data?.data) ? res.data.data.data.length : (Array.isArray(res.data?.data) ? res.data.data.length : 0);
      ...
      if (attendanceTemplates.length > 0) {
        const updatedAtt = await Promise.all(
          attendanceTemplates.map(async (t) => {
            try {
              // GET /:templateId/staff (no unassignedStaff param → returns assigned by default)
              const res = await axiosClient.get(`/attendanceonholiday-templates/${t.id}/staff`, { params: { unassignedStaff: false } });
  ...
  }, [holidayTemplates.length, attendanceTemplates.length]);
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deleteType === "Holiday") {
        await axiosClient.delete(`/holiday/${deleteModalId}`);
        setHolidayTemplates(prev => prev.filter(t => t.id !== deleteModalId));
      } else {
        await axiosClient.delete(`/attendanceonholiday-templates/${deleteModalId}`);
  ...
  const handleNewTemplateClick = () => {
    ...
    if (activeTab === "Holiday") {
      router.push("/dashboard/admin/hrms/others/holiday-policy/holiday-policy-form");
    } else {
      router.push("/dashboard/admin/hrms/others/holiday-policy/holiday-policy-attendance");
    }
  };

  const handleEdit = (template: any) => {
    sessionStorage.setItem("editHolidayPolicyId", template.id);
    setOpenMenuId(null);
    if (activeTab === "Holiday") {
      router.push("/dashboard/admin/hrms/others/holiday-policy/holiday-policy-form");
    } else {
  ...
  const displayedTemplates = activeTab === "Holiday" ? holidayTemplates : attendanceTemplates;
  ...
  return (
    <div className="flex flex-col min-h-screen px-4 py-3">
      ...
      <div className="flex items-center w-fit bg-[#f4f5f7] rounded-[8px] p-1 mb-6 border border-gray-200">
        {(["Holiday", "Attendance On Holiday"] as const).map(tab => (
          ...
          <>
            <div className="mb-[140px]">
              <h3 className="text-[16px] font-bold text-slate-800 mb-1 tracking-tight">
                {activeTab === "Holiday" ? "Holiday Template" : "Attendance on Holiday Templates"}
              </h3>
              <p className="text-[12px] text-gray-500 font-medium tracking-tight">
                {activeTab === "Holiday" ? "Create templates for holiday management" : "Create templates to mark attendance on holidays"}
              </p>
            </div>
            <div className="flex flex-col items-center justify-center -mt-6">
              <div className="mb-4"><Image src={cloudIcon} alt="No templates" className="w-[120px] h-auto object-contain" /></div>
              <p className="text-[10px] font-medium text-gray-400 mb-6 tracking-wide text-center">
                No templates found. Create one by clicking on the +New Template button
              </p>
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-[18px] font-bold text-slate-900 leading-tight">
                  {activeTab === "Holiday" ? "Holiday Templates" : "Attendance on Holiday Templates"}
                </h3>
                <p className="text-[12px] text-gray-400 mt-1 font-medium opacity-80">
                  {activeTab === "Holiday" ? "Manage your holiday templates" : "Manage your attendance on holiday templates"}
                </p>
              </div>
              <button onClick={handleNewTemplateClick}
                ...
                <div key={template.id}
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-3">
                      ...
                      {activeTab === "Holiday" && template.status === "ACTIVE" && (
                    ...
                    {activeTab === "Attendance On Holiday" && (
                      <p className="text-[12px] text-gray-400 font-medium">{template.attendanceType}</p>
                    )}
                    {activeTab === "Holiday" && (
                      <p className="text-[12px] text-gray-400 font-medium">{template.holidayCount} holiday{template.holidayCount !== 1 ? "s" : ""}</p>
                    )}
                  </div>
...
export default HolidayPolicyView;