...
import { Plus, MoreVertical, X, Search, Filter, User, ChevronDown } from "lucide-react";
...
const MultiSelectDropdown = ({
    ...
    const displayLabel = selected.size === 0
        ...
        : selected.size === 1
            ? [...selected][0]
            : `${[...selected][0]} & ${selected.size - 1} More`;
    ...
    return (
        <div className="mb-5" ref={ref}>
            ...
            >
                <span className={displayLabel ? "text-slate-800 font-medium" : "text-gray-400"}>
            ...
            {open && (
                <div className="mt-1 bg-white border border-gray-100 rounded-xl shadow-lg overflow-hidden z-[10] relative">
                    ...
                    >
                        <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${allSelected ? "bg-[#3f5a54] border-[#3f5a54]" :
                            someSelected ? "bg-[#3f5a54] border-[#3f5a54]" :
                    ...
                    {options.map(opt => (
                        ...
                        >
                            <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${selected.has(opt) ? "bg-[#3f5a54] border-[#3f5a54]" : "border-gray-300 bg-white"
                                }`}>
...
const FilterModal = ({
    ...
    onClose: () => void;
    onApply: (salaryTypes: Set<string>, shifts: Set<string>) => void;
}) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 animate-in fade-in duration-200" onClick={onClose}>
            <div
                className="bg-white w-full max-w-[480px] rounded-2xl shadow-2xl p-7 relative animate-in zoom-in-95 duration-200"
            ...
            >
                ...
                <MultiSelectDropdown
                    ...
                    selected={salaryTypes}
                    onChange={setSalaryTypes}
                />
                ...
                <div className="flex items-center gap-4 mt-6">
                    <button
                        onClick={() => { setSalaryTypes(new Set()); setShifts(new Set()); }}
                        className="flex-1 py-3 rounded-xl border-2 border-[#3f5a54] text-[#3f5a54] text-[14px] font-bold hover:bg-[#f3f6f5] transition-colors"
                    >
                        Clear Filter
                    </button>
                    <button
                        onClick={() => { onApply(salaryTypes, shifts); onClose(); }}
                        className="flex-1 py-3 rounded-xl bg-[#3e554d] text-white text-[14px] font-bold hover:bg-[#344641] transition-colors shadow-sm"
                    >
                        Apply Filter
                    </button>
                </div>
...
// ---- Staff List Modal (API-connected) ----
const StaffListModal = ({
    ...
    const [assignedStaff, setAssignedStaff] = useState<{ id: string; name: string; employeeCode: string; location: string }[]>([]);
    const [unassignedStaff, setUnassignedStaff] = useState<{ id: string; name: string; employeeCode: string; location: string }[]>([]);
    ...
    const loadStaff = useCallback(async (tid: string) => {
        ...
        try {
            const [aRes, uRes] = await Promise.all([
                axiosClient.get(`/shift-template/${tid}/staff`, { params: { unassignedStaff: false } }),
                axiosClient.get(`/shift-template/${tid}/staff`, { params: { unassignedStaff: true } }),
            ]);
            ...
            setAssignedStaff(extract(aRes).map(mapEmp));
            setUnassignedStaff(extract(uRes).map(mapEmp));
        } catch (err) {
    ...
    const filtered = staffPool.filter(
    ...
    const handleMoveToSelected = async () => {
        ...
        setOperating(true);
        try {
            await axiosClient.post("/shift-template/assign", {
                shiftTemplateId: templateId,
                employees: Array.from(checkedIds),
            });
            setCheckedIds(new Set());
    ...
    const handleMoveToUnselected = async () => {
        ...
        setOperating(true);
        try {
            await axiosClient.post("/shift-template/remove-employee", {
                shiftTemplateId: templateId,
                employees: Array.from(checkedIds),
            });
            setCheckedIds(new Set());
    ...
    return (
        <div className="fixed inset-0 z-[100] flex justify-end" onClick={onClose}>
            <div
                className="bg-white border border-gray-200 h-[calc(100%-32px)] my-4 mr-4 w-full max-w-[560px] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 rounded-2xl overflow-hidden"
                onClick={e => e.stopPropagation()}
            >
                {/* Header */}
                <div className="px-6 pt-6 pb-4 border-b border-gray-200">
                    <div className="flex items-start justify-between mb-1">
                        ...
                        <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-full transition-colors mt-0.5">
                            <X size={18} className="text-gray-500" />
                        </button>
                    </div>
                    ...
                    <div className="flex items-center w-fit bg-[#f4f5f7] rounded-[8px] p-1 mt-4 border border-gray-100">
                        {(["Selected", "Unselected"] as const).map(tab => (
                            <button
                                key={tab}
                                onClick={() => { setActiveStaffTab(tab); setCheckedIds(new Set()); }}
                                className={`px-4 py-1.5 rounded-[6px] text-[12px] font-medium transition-all duration-150 ${activeStaffTab === tab
                                    ? "bg-white text-slate-800 font-semibold shadow-[0_1px_3px_rgba(0,0,0,0.07)]"
                ...
                {showFilter && (
                    <FilterModal
                        ...
                        onApply={(salaryTypes, shifts) => {
                            console.log("Filters applied:", { salaryTypes, shifts });
                        }}
                    />
                )}
                    ...
                    <div className="border border-gray-100 rounded-xl overflow-hidden">
                        ...
                        <div className="grid grid-cols-[40px_1fr_120px_120px] items-center bg-gray-50 border-b border-gray-100 px-2 py-2.5">
                            <div className="flex justify-center">
                                <input
                                    ...
                                    checked={allChecked}
                                    onChange={toggleAll}
                                    className="w-3.5 h-3.5 accent-[#3f5a54] cursor-pointer rounded"
                                />
                            </div>
                        ...
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-16">
                                <p className="text-[13px] text-gray-400 font-medium">Loading staff...</p>
                            </div>
                        ) : filtered.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-16">
                            ...
                            filtered.map(staff => (
                                ...
                                >
                                    <div className="flex justify-center">
                                        <input
                                            type="checkbox"
                                            checked={checkedIds.has(staff.id)}
                                            onChange={() => { }}
                                            className="w-3.5 h-3.5 accent-[#3f5a54] cursor-pointer rounded pointer-events-none"
                                        />
                                    </div>
                ...
                <div className="px-6 py-5 border-t border-gray-100 flex justify-end">
                    {activeStaffTab === "Unselected" ? (
                        <button
                            ...
                            disabled={checkedIds.size === 0 || operating}
                            className={`px-6 py-2.5 rounded-xl text-[13px] font-semibold transition-all ${checkedIds.size > 0 && !operating
                                ? "bg-[#3e554d] text-white hover:bg-[#344641] shadow-sm"
                                : "bg-gray-100 text-gray-400 cursor-not-allowed"
                                }`}
                        >
                            {operating ? "Saving..." : "Move to Selected"}
                        </button>
                    ) : (
                        <button
                        ...
                        >
                            {operating ? "Saving..." : "Move to Unselected"}
                        </button>
                    )}
                </div>
            </div>
...
const ShiftSettingsView = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const shiftRes = await axiosClient.get("/shift-template");
                const rawShifts = shiftRes.data?.data || [];

                // Fetch assigned staff counts in parallel for all templates
                const staffCounts = await Promise.all(
                    rawShifts.map((t: any) =>
                        axiosClient
                            .get(`/shift-template/${t._id}/staff`, { params: { unassignedStaff: false } })
                            .then(res => {
                                const d = res.data?.data;
                                const list: any[] = Array.isArray(d?.data) ? d.data : Array.isArray(d) ? d : [];
                                return { id: String(t._id), count: list.length, ids: list.map((e: any) => String(e._id)) };
                            })
                            .catch(() => ({ id: String(t._id), count: 0, ids: [] }))
                    )
                );
                const countMap = Object.fromEntries(staffCounts.map(s => [s.id, s]));

                const mappedTemplates = rawShifts.map((t: any) => {
                    const sc = countMap[String(t._id)] || { count: 0, ids: [] };
                    return {
                        id: t._id || t.id,
                        templateName: t.name || t.templateName || "Untitled Shift",
                        shiftType: t.shiftType === "OPEN_SHIFT" ? "Open Shift" : "Fixed Shift",
                        startTime: t.startTime,
                        endTime: t.endTime,
                        workHours: t.totalWorkingMinutes
                            ? Math.floor(t.totalWorkingMinutes / 60) + ":" + (t.totalWorkingMinutes % 60).toString().padStart(2, "0")
                            : t.workHours,
                        assignedStaff: sc.count,
                        assignedStaffIds: sc.ids,
                    };
                });
    ...
    const handleDelete = async (id: string) => {
        try {
            await axiosClient.delete(`/shift-template/${id}`);
            const newList = templates.filter((t) => t.id !== id);
            setTemplates(newList);
            setDeleteModalId(null);
        } catch (err: any) {
            console.error("Failed to delete template:", err);
            setErrorMsg(err?.response?.data?.message || "Failed to delete template. Please try again.");
            setDeleteModalId(null);
        }
    };

    const handleNewTemplateClick = () => {
        sessionStorage.removeItem("editShiftTemplateId");
        router.push("/dashboard/admin/hrms/others/shift/new-shift");
    };

    const fixedShifts = templates.filter(t => t.shiftType === "Fixed Shift");
    const openShifts = templates.filter(t => t.shiftType === "Open Shift");

    return (
        <div className="flex flex-col min-h-screen px-4 py-3">
            {/* Main Content Card */}
            <div className="bg-white rounded-2xl border border-gray-200 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.02)] p-8 flex flex-col min-h-[500px]">
                {templates.length === 0 ? (
                    // === EMPTY STATE ===
                    <>
                        ...
                        <div className="flex flex-col items-center justify-center -mt-6">
                            ...
                            <p className="text-[10px] font-medium text-gray-400 mb-6 tracking-wide text-center">
                                No templates found. Create a Shift template by clicking on the +New Template button
                            </p>
                        ...
                        <div className="flex flex-col gap-6">
                                ...
                                <div className="flex flex-col gap-4">
                                    <div className="mb-2">
                                        <div className="flex items-center gap-2 mb-1">
                                            <h3 className="text-[16px] font-bold text-slate-800">Fixed Shifts</h3>
                                            <span className="bg-gray-300 text-gray-500 text-[12px] font-bold w-5 h-5 rounded-full flex items-center justify-center">
                                                {fixedShifts.length}
                                            </span>
                                        </div>
                                        <p className="text-[12px] text-gray-500 font-medium tracking-tight">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        {fixedShifts.map(template => (
                                            ...
                                            >
                                                ...
                                                {/* Right Side: Actions */}
                                                <div className="flex items-center gap-8 text-[12.5px] font-medium text-gray-500 transition-colors">
                                                    <div
                                                        onClick={() => { setAssignStaffTemplateId(template.id); setOpenMenuId(null); }}
                                                        className="tracking-tight opacity-90 cursor-pointer hover:text-[#3e554d] hover:underline transition-colors"
                                                    >
                                                        Assigned Staff: {template.assignedStaff}
                                                    </div>
                                                    <div className="relative">
                                                        <button
                                                            ...
                                                            className={`p-1.5 rounded-lg transition-colors ${openMenuId === template.id
                                                                ? "bg-gray-100 text-[#3e554d]"
                                                                : "text-gray-400 hover:bg-gray-50"
                                                                }`}
                                                        >
                                                            <MoreVertical size={18} className="text-slate-800" />
                                                        </button>

                                                        {openMenuId === template.id && (
                                                            <div
                                                                ref={menuRef}
                                                                className="absolute right-0 mt-2 w-[140px] bg-white border border-gray-100 rounded-[10px] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] z-50 py-1 flex flex-col animate-in fade-in zoom-in-95 duration-200"
                                                            >
                                                                <button
                                                                    onClick={() => {
                                                                        sessionStorage.setItem("editShiftTemplateId", template.id);
                                                                        setOpenMenuId(null);
                                                                        router.push("/dashboard/admin/hrms/others/shift/new-shift");
                                                                    }}
                            ...
                            {openShifts.length > 0 && (
                                <div className="flex flex-col gap-4 mt-2">
                                    <div className="mb-2">
                                        <div className="flex items-center gap-2 mb-1">
                                            <h3 className="text-[16px] font-bold text-slate-800">Open Shifts</h3>
                                            <span className="bg-gray-300 text-gray-500 text-[12px] font-bold w-5 h-5 rounded-full flex items-center justify-center">
                                                {openShifts.length}
                                            </span>
                                        </div>
                                        ...
                                        {openShifts.map(template => (
                                            ...
                                            >
                                                ...
                                                <div className="flex flex-col gap-1.5">
                                                    ...
                                                    <p className="text-[12.5px] text-gray-500 font-medium tracking-tight">
                                                        Duration: {template.workHours} Hr
                                                    </p>
                                                </div>
            ...
            <StaffListModal
                ...
                onDone={() => {
                    // Refresh assigned count from server after a move
                    if (!assignStaffTemplateId) return;
                    axiosClient.get(`/shift-template/${assignStaffTemplateId}/staff`, { params: { unassignedStaff: false } })
                        .then(res => {
                            const d = res.data?.data;
                            const list: any[] = Array.isArray(d?.data) ? d.data : Array.isArray(d) ? d : [];
                            setTemplates(prev => prev.map(t =>
                                t.id === assignStaffTemplateId
                                    ? { ...t, assignedStaff: list.length, assignedStaffIds: list.map((e: any) => e._id) }
                                    : t
                            ));
                        })
                        .catch(console.error);
...
export default ShiftSettingsView;